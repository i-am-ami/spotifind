...
- `npm run test:search` / `npm run test:server` / `npm run test:lrclib` — run the unit/integration test suites
...
| `tests/lrclib.test.js` | Tests for `backend/lrclib.js`'s HTTP response handling and LRC parsing, using `node:test`'s `mock.method` to stub `globalThis.fetch` (no real network calls, no new dependency) |
...
- **2026-09-12** — Added `parseSyncedLyrics` to `backend/lrclib.js`, parsing LRCLIB's line-synced `syncedLyrics` (LRC format) into `{ time, text }` pairs, stored alongside plain lyrics in `data/lyrics.json` via `lyrics.js`. First step toward the "scrub to the matched lyric in Spotify" idea — not wired into search or playback yet. Wrote `tests/lrclib.test.js` (17 cases) using `node:test`'s `mock.method` to stub `fetch`, since `undici` isn't importable as a bare/built-in module on this Node version without adding it as a real dependency.

## Roadmap

1. Harden the backend before adding new features. `search.js`/`server.js`/`lrclib.js` test coverage done; next is extracting `songs.js`/`lyrics.js`'s pure dedup/filter logic for testing.
...
- Jump Spotify playback to the matched lyric's timestamp. Line-synced data is now fetched and parsed (see timeline above); actually seeking playback still needs Spotify's playback-control API (Premium + active device + broader OAuth scopes than we request today), plus wiring a matched search result's line back to its timestamp.
